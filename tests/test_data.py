...
class TestChestXRayDataset:
    ...
    def test_dataset_getitem_image_shape(self, mock_data_dir, expected_shape):
        ...
        assert image.shape == expected_shape, f"Image should have shape {expected_shape}, got {image.shape}"
    ...
    def test_dataset_labels_count(self, mock_data_dir):
        ...
        expected_count = 5
        assert normal_count == expected_count, f"Should have {expected_count} NORMAL samples, got {normal_count}"
        assert (
            pneumonia_count == expected_count
    ...
    def test_dataset_samples_are_sorted(self, mock_data_dir):
        ...
        assert sample_names == sorted(sample_names), "Sample paths should be sorted for reproducibility"
    ...
    def test_dataset_missing_class_directory(self):
        ...
        assert all(
            label == 0 for label in dataset.labels
    ...
    def test_dataset_transform_grayscale_to_rgb(self, mock_data_dir):
        ...
        assert image.shape[0] == 3, f"Images should have 3 channels (RGB), got {image.shape[0]} channels"
    ...
    def test_dataset_preprocess_creates_directories(self):
        ...
        assert (output_folder / "train" / "NORMAL" / "test.jpeg").exists(), "Data should be copied to processed folder"
