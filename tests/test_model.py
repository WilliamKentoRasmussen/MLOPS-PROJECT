...
class TestBaselineCNN:
    ...
    def test_baseline_output_shape(self):
        ...
        assert output.shape == (1, 2), \
            f"Output shape should be (1, 2) for binary classification, got {output.shape}"
    ...
    def test_baseline_different_input_sizes(self, input_size):
        ...
        assert output.shape == (1, 2), \
            f"Output shape should be (1, 2) regardless of input size, got {output.shape}"
...
class TestGetModel:
    ...
    def test_get_model_returns_module(self, model_name):
        ...
        assert isinstance(model, nn.Module), \
            f"get_model('{model_name}') should return an nn.Module instance"
    ...
    def test_get_model_num_classes(self, num_classes):
        ...
        assert output.shape[1] == num_classes, \
            f"Output should have {num_classes} classes, got {output.shape[1]}"
    ...
    def test_get_model_pretrained_flag(self, model_name):
        ...
        assert isinstance(model_no_pretrain, nn.Module), \
            f"Model '{model_name}' with pretrained=False should be valid"
        
        # Test with pretrained=True (only for alexnet and vgg16)
        if model_name != "baseline":
            model_pretrained = get_model(model_name, pretrained=True)
            assert isinstance(model_pretrained, nn.Module), \
                f"Model '{model_name}' with pretrained=True should be valid"
    ...
    def test_model_device_transfer(self, model_name):
        ...
        assert output_cpu.device.type == 'cpu', \
            f"Model '{model_name}' output should be on CPU"
        ...
        if torch.cuda.is_available():
            ...
            assert output_cuda.device.type == 'cuda', \
                f"Model '{model_name}' output should be on CUDA"
