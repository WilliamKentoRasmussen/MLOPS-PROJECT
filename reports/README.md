...
We used uv for managing our dependencies, because of its increasing relevance in the field. All project dependencies are declared in the ```pyproject.toml```, while exact resolved versions are pinned in the ```uv.lock``` generated by uv. These files ensure reproducibility across machines and development stages.

To obtain a complete copy of our development environment, a new team member would first install uv and ensure a compatible version of python (specified in ```.python-version```). After cloning the repository, they would run ```uv sync```, which automatically creates a local virtual (if it does not already exist) environment and installs all dependencies exactly as specified in the lockfile. From there, they can run scripts with the same package versions are the rest of the team
...
In total, we implemented 5 tests, including unit tests for data processing and model training, as well as integration and performance tests for the backend and frontend of our API. The data and model tests verify correct data splitting, input and output shapes, and that models are trained and stored correctly. For the API, we tested backend functionality such as model loading and prediction, frontend connectivity to the backend, and load performance under multiple users using Locust.
...
We managed to deploy our API locally and in the cloud. For both deployments, the model was wrapped our model in a FastAPI framework.Local deployment was primarily used during development and testing, allowing us to quickly validate functionality before moving to the cloud. 

For cloud deployment, We deployed it in the cloud using the Cloud Run Service. The FastAPI application was packaged into a docker image and pushed to the artifact registry, after which it was deployed. 

The deployed service can be invoked using ```curl```request, for example:  
```"curl -X POST -F "file=@file.json"https://mlops-prediction-function-1031863562600.europe-west1.run.app/predict```. 

Additionally, it can also be run using the docs with this url https://mlops-prediction-function-1031863562600.europe-west1.run.app/docs.
...
The most expensive service was Compute Engine, due to its relatively high monthly rate and its use in training our model (as well as us forgetting to shut down multiple instances when they were inactive). In constrast, Cloud run was surprisngly in expensive while still supporting our backend deployment. Overall, working in the cloud was challenging but very insightful, and it helped us better understand cloud-based workflows and resource management. If we ere to redo the project, we would follow a more structed apporach and rely more on official documentation to avoid unnecessary debugging loops.
...
We overcame these challenges through debugging, group discussions, and incremental improvements. Overall, although the project was challenging, these struggles provided valuable hands-on experience with real-world MLOps complexity. It also highlighted the importance of planning, clear task division, and prioritizing core functionality early in the development process
